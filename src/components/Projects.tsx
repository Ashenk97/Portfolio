...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  const cardClass = `flex h-full flex-col rounded-2xl border p-6 no-underline ${
    isFeatured
      ? 'border-ember/40 bg-gradient-to-br from-panel to-ink'
      : 'border-line bg-panel/70'
  } ${project.href ? 'transition hover:border-ember/70' : ''}`;
  ...
  const body = (
    <>
      ...
      <h3 className="mt-5 text-2xl font-bold tracking-tight text-paper">{project.name}</h3>
    ...
    <FadeIn delay={index * 0.08} className="h-full">
      <TiltCard className="h-full">
        {project.href ? (
          ...
          <article data-project={project.slug} className={cardClass}>
            {body}
          </article>
        )}
      </TiltCard>
